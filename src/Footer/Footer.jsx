...
function Cart({title}) {
    return (
      <span className='w-[44%]  lg:w-[20%] text-sm text-start cursor-pointer hover:scale-[1.05] transition duration-1000 ease-in-out'>{title}</span>
    )
}
    ...
    return (
      ...
      <div className='mb-10'>
        ...
        <div className='flex flex-wrap justify-between gap-y-5 lg:gap-y-2 grid-rows-4 mb-10'>
            ...
            <Cart title={"Legal Notics"} />
            <Cart title={"Only On netflix"} />
        </div>

        <button className='mb-5 bg-blue-900  text-white text-md px-3 py-2 rounded-md hover:scale-[1.1] transition ease-in-out duration-700'>Watch Videos</button>

        <img className='w-40 lg:w-60 mb-10' src={logo} alt="" />
      </div>

        <div className='border-t-[0.5px] border-gray-300 px-2 lg:px-5 pt-3'>
