function Cart({ heading, title }) {
    return (
      <div className="relative group">
        <div className="bg-[#4a4a4d] px-5 py-8 flex flex-col gap-3 rounded-lg border-[0.5px] border-gray-500 mb-10 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg">
          <h1 className="text-xl">{heading}</h1>
          <p>{title}</p>
        </div>
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-500 to-purple-500 opacity-0 group-hover:opacity-50 transition-opacity duration-300"></div>
      </div>
    );
  }
  ...
  export default function FullPage_Screen4() {
    return (
      <div className="bg-gray-300 border font-sans min-h-[200vh] py-20 px-5">
        <div className="bg-black text-white h-full rounded-lg px-20 py-10">
          <h2 className="text-5xl mb-10 text-center font-bold tracking-wide">Discover Your Personalized Watchlist</h2>
  ...
  function HoverableCart({ heading, title, color }) {
    return (
      <div className="group">
        <div className={`relative bg-gradient-to-br ${color} px-5 py-8 flex flex-col gap-3 rounded-lg border border-gray-500 mb-10 transition duration-300 ease-in-out transform group-hover:scale-105 group-hover:shadow-lg`}>
